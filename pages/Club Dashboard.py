...
st.markdown(
    """
    <style>
      /* widen main container so fixed-width charts fit on Cloud */
      .block-container {
        max-width: 1400px;
      ...
      /* align chart containers to the right if you wrap them */
      .chart-container { display: flex; justify-content: flex-end; width: 100%; margin-bottom: 2rem; }
    </style>
    """,
...
# Sidebar “Status” filter (sidebar collapsed by default)
...
FROM (
  ...
  FROM users u
  WHERE u.club_id = (SELECT id FROM clubs WHERE name = %(club_name)s)
...
if df_age.empty:
    ...
    bar_age = (
        Bar(init_opts=opts.InitOpts(width="1200px", height="500px", bg_color="#111111"))
        .add_xaxis(labels)
        .add_yaxis(
            "Number of Skiers", values, category_gap="25%",
            label_opts=opts.LabelOpts(
                color="#ffffff",
                formatter=JsCode("function(p){return p.value>5 ? p.value : ''}")
            )
        )
        .set_global_opts(
            xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(color="#ffffff", rotate=45)),
    ...
    html(bar_age.render_embed(), width=1200, height=500, scrolling=False)

# ─── 3) Current Level Distribution (Horizontal) ────────────
...
WHERE
  ...
  AND u.role   = 'skier'
  AND u.active IS TRUE
GROUP BY u.current_level, level_name
ORDER BY u.current_level;
"""
df_current = pd.read_sql(sql_current_level, engine, params={"club_name": club_choice})
if df_current.empty:
    ...
    bar_current = (
        Bar(init_opts=opts.InitOpts(width="1200px", height="400px", bg_color="#111111"))
        .add_xaxis(levels)
        .add_yaxis("Skiers", counts, category_gap="35%",
    ...
    html(bar_current.render_embed(), width=1200, height=400, scrolling=False)

# ─── 4) Evaluations Passed by Level (Club) ─────────────────
sql_eval_dist = """
...
df_eval_dist = df_eval_dist[df_eval_dist["sort_ord"].notna()].sort_values("sort_ord")
    ...
    bar = (
        ...
        .set_global_opts(
            ...
            toolbox_opts=opts.ToolboxOpts(feature={
                "saveAsImage": {}, "restore": {}, "dataZoom": {}, "dataView": {}, "magicType": {}
            })
        )
    )
    html(bar.render_embed(), width=1200, height=400, scrolling=False)

# ─── 5) Pass-Rate % by Level (Club) ───────────────────────
...
GROUP BY level_id, level_name;
"""
df_pass_club = pd.read_sql(sql_pass_rate_club, engine, params={"season": target_season, "club_name": club_choice})
...
if df_pass_club.empty:
    ...
    pie_pass_club = (
        Pie(init_opts=opts.InitOpts(width="1200px", height="600px", bg_color="#111111"))
        .add(
            "Pass %",
            data_pairs,
            radius=["15%", "65%"],
    ...
    html(pie_pass_club.render_embed(), width=1200, height=600, scrolling=False)

# ─── 6) Skiers list for the selected club ──────────────────
...
"""
df_skiers = pd.read_sql(sql_skiers, engine, params={"club_name": club_choice})
if df_skiers.empty:
