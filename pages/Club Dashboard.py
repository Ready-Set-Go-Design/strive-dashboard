...
st.markdown(
    ...
    <style>
      .block-container {
        max-width: 1600px; /* more width on Cloud */
...
ORDER BY level_id;
"""
df_dist_full = pd.read_sql(sql_dist_full, engine, params={
    "season": season, "ptso": ptso_sel, "names": club_sel
})
...
WHERE season    = %(season)s
  ...
  AND club_name = ANY(%(names)s);
"""
df_sum = pd.read_sql(sql_sum, engine, params={
    "season": season, "ptso": ptso_sel, "status": status_sel, "names": club_sel
})
if df_sum.empty:
...
GROUP BY level_id, level_name;
"""
df_eval_full = pd.read_sql(sql_eval_full, engine, params={
    "season": season, "ptso": ptso_sel, "names": club_sel
})
...
if df_dist.empty:
    ...
    pie = (
        Pie(init_opts=opts.InitOpts(width="100%", height="420px", bg_color="#111111"))
        .add("", df_dist[["level_name", "skier_count"]].values.tolist(),
             radius=["40%", "70%"])
        .set_global_opts(
            legend_opts=opts.LegendOpts(orient="horizontal", pos_top="2%",
                                        textstyle_opts=opts.TextStyleOpts(color="#ffffff")),
            toolbox_opts=opts.ToolboxOpts(feature={"saveAsImage": {}, "restore": {}, "dataZoom": {},
                                                   "dataView": {}, "magicType": {}})
        )
...
if df_eval.empty:
    ...
    bar = (
        Bar(init_opts=opts.InitOpts(width="100%", height="420px", bg_color="#111111"))
...
if _df_clubs.empty:
    ...
    st.markdown("""<style>.ag-root-wrapper, .ag-theme-streamlit { width: 100% !important; }</style>""",
                unsafe_allow_html=True)
    ...
    AgGrid(df_display, gridOptions=grid_options, theme="streamlit", fit_columns_on_grid_load=True)
